...
def morphImage():
    ...
    inImage2 = np.fromfile(filename2, dtype=np.uint8).reshape(inH2, inW2)
...
def stretchImage3():  # Equalization
    ...
    # Main Code
    inCountList = np.zeros(256)
    for i in range(inH):
        for k in range(inW):
            inCountList[inImage[i][k]] += 1

    cumCount = np.zeros(256)
    cumCount[0] = inCountList[0]
    for i in range(1, 256):
        cumCount[i] += cumCount[i - 1] + inCountList[i]

    normalCount = np.zeros(256)
    for i in range(256):
        normalCount = cumCount * (1 / (inH * inW)) * 255
    normalCount = normalCount.astype(np.int32)
...
def zoomOutImage():
    ...
    print(outH, inH)
    # Memory Allocation
    outImage = np.zeros(outH * outW)
    print(len(outImage))
    # Main Code
    tmpImage = inImage.reshape((inH * inW,))
    print(len(tmpImage))
    k = 0
    for i in range(0, len(tmpImage), scale * scale):
        outImage[k] = np.mean(tmpImage[i:i+scale])
        k += 1
    print(outH, outW)
    outImage = outImage.reshape((outH, outW)).astype(np.uint8)
...
comVisionMenu2.add_command(label="Zoom Out", command=zoomOutImage)
