...
def saveImageColor():
    ...
    saveFp = asksaveasfile(parent=window, mode='wb', defaultextension='.',
                           filetypes=(("Image File", "*.jpg;*.png;*.bmp;*.tif"),  ("All File", "*.*")))
    outArray = []
    for i in range(outH):
        tmpList = []
        for k in range(outW):
...
def displayImageColor():
    global window, canvas, paper, filename, inImage, outImage, inW, inH, outW, outH
    global VIEW_X, VIEW_Y, stepX, stepY
    ...
    for i in np.arange(0, outH, stepY):
        tmpStr = ""
        for k in np.arange(0, outW, stepX):
            i = int(i); k = int(k)
...
def satImagePIL():
    ...
    # Attention: display size required!!
    value = askfloat("Saturation", "Enter the value(0~1~10):")
    ...
    photo2 = photo.copy()
    photo2 = ImageEnhance.Color(photo2).enhance(value)
    # Memory Allocation
    outImage = []
    for _ in range(3):
        outImage.append(malloc(outH, outW))
...
def satImageHSV():
    global window, canvas, paper, filename, inImage, outImage, inW, inH, outW, outH
    ## Input RGB --> Input HSV
    # Memory Allocation for HSV Model
    inImageHSV = []
    for _ in range(3):
    ...
    for i in range(inH):
        for k in range(inW):
            ...
            h, s, v = colorsys.rgb_to_hsv(r / 255, g / 255, b / 255)  # The value should be between 0 ~ 1
    ...
    # Main Code
    value = askfloat("Saturation", "Enter the value(-255 ~ 255)")
    value /= 255
    ...
    for i in range(outH):
        for k in range(outW):
            ...
            outImage[R][i][k], outImage[G][i][k], outImage[B][i][k] = map(int, [r, g, b])
...
def emboImageHSV():
    ...
    global sx, sy, ex, ey
    ## Mouse Event Binding
    canvas.bind("<Button-3>", rightMouseClick_emboImageHSV)
    canvas.bind("<Button-1>", leftMouseClick)
    canvas.bind("<B1-Motion>", leftMouseMove)
    canvas.bind("<ButtonRelease-1>", leftMouseDrop_emboImageHSV)
    canvas.configure(cursor='mouse')

def leftMouseDrop_emboImageHSV(event):
    global window, canvas, paper, filename, inImage, outImage, inW, inH, outW, outH
    global sx, sy, ex, ey
    sx *= stepX
    sy *= stepY
...
def leftMouseMove(event):
    global window, canvas, paper, filename, inImage, outImage, inW, inH, outW, outH
    global sx, sy, ex, ey, boxLine, stepX, stepY
    if not boxLine: # Check if the line has been drawn before
        pass
    else:
...
def rightMouseClick_emboImageHSV(event):
    global window, canvas, paper, filename, inImage, outImage, inW, inH, outW, outH
    global sx, sy, ex, ey
    sx, sy = 0, 0
    ex, ey = inW-1, inH-1
...
def __emboImageHSV():
    ...
    # Allocate empty memory for temp input image
    tmpInImageV = malloc(inH + (MSize - 1), inW + (MSize - 1), 128)
    ...
    for i in range(outH):
        for k in range(outW):
            value = tmpOutImageV[i][k] + 128
            if value > 255:
                value = 255
            elif value < 0:
                value = 0
            tmpOutImageV[i][k] = value

    ## HSV --> RGB
    for i in range(outH):
        for k in range(outW):
            if sx <= k <= ex and sy <= i <= ey: # if
...
comVisionMenu1.add_command(label="Saturation(PIL)", command=satImagePIL)
comVisionMenu1.add_command(label="Saturation(HSV)", command=satImageHSV)
...
comVisionMenu4.add_command(label="Embossing(RGB)", command=emboImageColor)
comVisionMenu4.add_command(label="Embossing(HSV)", command=emboImageHSV)
comVisionMenu4.add_command(label="Embossing(PIL)", command=emboImagePIL)
fileMenu.add_separator()
comVisionMenu4.add_command(label="Blurring(RGB)", command=blurImageColor)
